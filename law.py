...
class Law:
    ...
    def __parse(self):
        ...
        while True:
            ...
            if mark is not None:
                self.jangs.append(Jang(Helper.pages_extract(self.raw, temp_mark, mark), temp_mark, self.counter))
                temp_mark = mark
            else:
                self.jangs.append(Jang(Helper.pages_extract_til_end(self.raw, temp_mark), temp_mark, self.counter))
                temp_mark = Mark(0,0)
                break
            index += 1
